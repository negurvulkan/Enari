...
final class AdminWorkspace
{
    ...
    private function normalizeConfig(array $adminConfig): array
    {
        ...
        $versionLabel = trim((string) ($adminConfig['versionLabel'] ?? 'v1.4'));
        ...
        return array(
            ...
            'versionLabel' => $versionLabel !== '' ? $versionLabel : 'v1.4',
    ...
    private function renderAppPage(): void
    {
        ...
        $title = (string) ($this->config['title'] ?? 'CMS Workspace');
        $versionLabel = (string) ($this->config['versionLabel'] ?? 'v1.4');
        $adminTheme = $this->resolveAdminThemeKey();
        ...
        echo '<section class="admin-panel"><div class="admin-panel__header"><h3>Content Sync</h3><div class="admin-inline-actions"><button type="button" class="admin-button admin-button--ghost admin-button--small" data-admin-git-fetch>Fetch</button><button type="button" class="admin-button admin-button--ghost admin-button--small" data-admin-git-pull>Pull</button><button type="button" class="admin-button admin-button--ghost admin-button--small" data-admin-git-push>Push</button></div></div><div class="admin-panel__body"><div class="admin-git-stack"><div class="admin-git-summary" data-admin-git-summary><p class="admin-placeholder">Git-Status des Content-Repositories wird geladen.</p></div><label class="admin-field"><span>Commit-Message</span><textarea rows="3" data-admin-git-commit-message placeholder="z. B. Update phonology notes"></textarea></label><label class="admin-field"><span>Validierung vor Commit/Push</span><select data-admin-git-validation><option value="content">Content/i18n</option><option value="release">Release-Check</option><option value="none">Keine</option></select></label><div class="admin-inline-actions"><button type="button" class="admin-button admin-button--ghost admin-button--small" data-admin-git-setup>Content-Remote einrichten</button><button type="button" class="admin-button admin-button--primary admin-button--small" data-admin-git-commit>Commit</button><button type="button" class="admin-button admin-button--ghost admin-button--small" data-admin-git-merge-open hidden>Merge fortsetzen</button></div><div data-admin-git-files><p class="admin-placeholder">Noch keine Git-Dateiliste geladen.</p></div><div data-admin-git-queue><p class="admin-placeholder">Noch keine Sync-Hinweise vorhanden.</p></div></div></div></section>';
        echo '</section>';
